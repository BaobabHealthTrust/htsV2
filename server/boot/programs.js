...
module.exports = function(app) {
  ...
  const HtsRegisterServiceDeliveryPoint =
    app.models.HtsRegisterServiceDeliveryPoint;
  ...
  const htsIndicatorsMapping = require(__dirname +
    "/../../configs/htsIndicatorsMapping.json");
      ...
      value: function(format) {
        ...
        if (format.match(/YYYY-mm-dd\sHH:MM:SS/)) {
          ...
          result =
            date.getDate() +
            " " +
            monthNames[parseInt(date.getMonth())] +
            ", " +
            date.getFullYear();
          ...
          result =
            date.getDate() +
            " " +
            months[parseInt(date.getMonth(), 10)] +
            " " +
            date.getFullYear();
        } else {
          result =
  ...
  const validActiveToken = token => {
    return new Promise((resolve, reject) => {
      ...
      User.accessToken.findById(token, (err, result) => {
        debug(result);

        if (result) {
          const ttl =
            (!isNaN(result.ttl) ? parseInt(result.ttl, 10) : 0) * 1000;
  ...
  const fetchAge = json => {
    if (json.dateOfBirth) {
      try {
        let age =
          new Date().getFullYear() - new Date(json.dateOfBirth).getFullYear();

        if (age <= 2) {
          age = (
            (new Date() - new Date(json.dateOfBirth)) /
            (30 * 24 * 60 * 60 * 1000)
          ).toFixed(0);
        ...
        json.age = age + (!String(age).match(/[M|W|D]/) ? "Y" : "");
  ...
  const generateId = async (
    ...
    return await mutex
      ...
      .then(async unlock => {
        ...
        const yr =
          new Date().getMonth() < months[site["reset month"].toLowerCase()]
            ? new Date().getFullYear() - 1
            : new Date().getFullYear();

        const property =
          (prefix ? prefix : "DMY").trim().toLowerCase() +
        ...
        const nextId =
          result.length > 0 ? parseInt(result[0].propertyValue, 10) + 1 : 1;
        ...
        let locationId = location ? location.locationId : 1;
        ...
        let creator = user ? user.id : 1;

        let idType = await PatientIdentifierType.find({
          where: {
            name:
              !prefix || (prefix && prefix == "DMY")
                ? "Dummy ID"
                : prefix.trim() + " Number"
          },
          limit: 1
        ...
        let identifierType =
          idType.length > 0 ? idType[0].patientIdentifierTypeId : 8;
    ...
    return await mutex
      ...
      .then(async unlock => {
        const padded =
          ...
          padZeros(
            0,
            site.entry_code_max_digits ? site.entry_code_max_digits : 3
          );

        const largestPossibleId = parseInt(padded, 10) - 1;

        const property =
          (prefix ? prefix : "EC").trim().toLowerCase() + ".id.counter";
        ...
        let identifier =
          prefix.trim().toUpperCase() + "" + nextId + "-" + check;
        ...
        let conceptId = concept ? concept.conceptId : null;
    ...
    return new Promise(resolve => {
      new client().get(
        req.protocol +
          ...
          "/dde/search_by_identifier/" +
          raw.npid,
        function(result, props) {
          ddeData =
            result.data && result.data.matches === 1
              ? result.data.hits[0]
              : result;
  ...
  const addNewPatient = (req, raw) => {
    ...
    return new Promise(resolve => {
      if (
        Object.keys(raw).indexOf("birthdate") < 0 &&
        Object.keys(raw).indexOf("Date of Birth") < 0
      )
        return resolve();

      const args = {
        data: {
          family_name: raw.names ? raw.names.family_name : raw["Last Name"],
          given_name: raw.names ? raw.names.given_name : raw["First Name"],
          middle_name: raw.names ? raw.names.middle_name : raw["Middle Name"],
          gender: raw.gender ? raw.gender : raw.Gender,
          attributes: {
          ...
          birthdate: raw.birthdate ? raw.birthdate : raw["Date of Birth"],
          identifiers: {
            "Entry Code": raw.otherId ? raw.otherId : null
          },
          birthdate_estimated:
            Object.keys(raw).indexOf("birthdate_estimated") >= 0
              ? raw.birthdate_estimated
              : raw["Birthdate Estimated?"] &&
          ...
          home_village: raw.addresses
            ...
            : raw["Home Village"],
          home_ta: raw.addresses ? raw.addresses.home_ta : raw["Home T/A"],
      ...
      new client().post(
        ...
        args,
        function(result, props) {
          ddeData = result.data ? result.data : result;
    ...
    if (raw["Age"]) {
      ...
      if (parts) {
        ...
        switch (unit.toUpperCase()) {
          case "D":
            raw["Date of Birth"] = new Date(
              new Date().setDate(new Date().getDate() - parseInt(number, 10))
            );
          ...
          case "W":
            raw["Date of Birth"] = new Date(
              new Date().setDate(
                new Date().getDate() - parseInt(number, 10) * 7
              )
            );
          ...
          case "M":
            raw["Date of Birth"] = new Date(
              new Date().setDate(
                new Date().getDate() - parseInt(number, 10) * 30
              )
            );
          ...
          default:
            raw["Date of Birth"] =
              new Date().getFullYear() - parseInt(number, 10) + "-07-10";

            break;
        }
      } else {
        raw["Date of Birth"] =
          new Date().getFullYear() - parseInt(raw["Age"], 10) + "-07-10";
      }
  ...
  router.get("/programs", function(req, res, next) {
    ...
    const role = query.role ? query.role : "regular";
      ...
      folders
        ...
        .filter(f => {
          switch (role.toLowerCase()) {
            case "admin":
              ...
              return (
                f.toLowerCase().trim() !== "registration" &&
                f.toLowerCase().trim() !== "user management"
              );
        ...
        .forEach(folder => {
          ...
          if (!userDashTasks || (userDashTasks && userDashTasks.length <= 0)) {
            userDashTasks = JSON.parse(
              fs.readFileSync(userFile).toString("utf8")
            )["regular"];
          }
  ...
  router.post("/programs/save_demographics", async function(req, res, next) {
    ...
    if (!raw["Create local with given ID?"]) {
      if (raw.npid) {
        ...
        if (ddeData && !ddeData.data) {
          ...
          ddeData.data &&
          ddeData.data.hits &&
          Array.isArray(ddeData.data.hits)
        ) {
          ddeData.data.hits.forEach(async json => {
            if (Object.keys(json).indexOf("patientName") < 0) {
              json.patientName =
                (json.names && json.names.given_name
            ...
            if (Object.keys(json).indexOf("currentVillage") < 0) {
              json.currentVillage =
                json.addresses && json.addresses.current_village
                  ? json.addresses.current_village
                  : "";

              json.currentTA =
                json.addresses && json.addresses.current_ta
                  ? json.addresses.current_ta
                  : "";

              json.currentDistrict =
                json.addresses && json.addresses.current_district
            ...
            if (Object.keys(json).indexOf("cellPhoneNumber") < 0) {
              json.cellPhoneNumber =
                json.attributes && json.attributes.cell_phone_number
            ...
            let patientId = identifier ? identifier.patientId : null;
          ...
          return res.status(200).json(ddeData.data.hits);
        } else {
      ...
      raw["Create local with given ID?"] &&
      raw["Create local with given ID?"] === "No"
    ) {
      return res.status(200).json([]);
    }
    ...
    let patient =
      [undefined, null].indexOf(primaryId) < 0
    ...
    const patientIdentifierType = await PatientIdentifierType.findOne({
      where: { name: "National id" }
    }).catch(e => {
      return null;
    });

    const npidIdentifierTypeId =
      patientIdentifierType !== null
        ? patientIdentifierType.patientIdentifierTypeId
        : null;
    ...
    let json = {
      ...
      npid:
        raw.npid &&
        patient !== null &&
        npidIdentifierTypeId === patient.identifierType
          ? raw.npid
          : "",
      age: raw.birthdate
      ...
      patientName: raw.names
        ...
        ? raw["First Name"] + " " + raw["Last Name"]
        : "- -",
      firstName: raw.names ? raw.names.given_name : raw["First Name"],
      lastName: raw.names ? raw.names.family_name : raw["Last Name"],
      dateOfBirth: raw.birthdate ? raw.birthdate : raw["Date of Birth"],
      districtOfOrigin: raw.addresses
        ? raw.addresses.home_district
        : raw["District of Origin"],
      taOfOrigin: raw.addresses ? raw.addresses.home_ta : raw["T/A of Origin"],
      ...
      currentDistrict: raw.addresses
        ? raw.addresses.current_district
        : raw["Current District"],
      currentTA: raw.addresses ? raw.addresses.current_ta : raw["Current T/A"],
    ...
    if (!patient) {
      ...
      let result = await PersonName.create({
        ...
        middleName: json.middleName ? json.middleName : null,
      ...
      let locationId = location.length > 0 ? location[0].locationId : 1;
      ...
      if (raw.npid) {
        ...
        identifierType =
          idType.length > 0 ? idType[0].patientIdentifierTypeId : 3;
        ...
        if (raw.docId) {
          ...
          identifierType =
            idType.length > 0 ? idType[0].patientIdentifierTypeId : 28;
        ...
        identifierType =
          idType.length > 0 ? idType[0].patientIdentifierTypeId : 10;
      ...
      let patientId = patient ? patient.patientId : null;
  ...
  router.post("/programs/fetch_json", function(req, res, next) {
    ...
    if (req.body.path && fs.existsSync(req.body.path)) {
      ...
      const configsPath = req.body.path.replace(
        /[^\/]+$/,
        (result.subGroup === "PatientRegistration"
          ? "config/Registration"
          : "config") + "/app.json"
      );

      if (fs.existsSync(configsPath)) {
        result.configs = JSON.parse(
          fs.readFileSync(configsPath).toString("utf8")
        );
      }

      const ignoresPath = req.body.path.replace(
        /[^\/]+$/,
        (result.subGroup === "PatientRegistration"
          ? "config/Registration"
      ...
      if (fs.existsSync(ignoresPath)) {
        result.ignores = JSON.parse(
          fs.readFileSync(ignoresPath).toString("utf8")
        );
      }

      const referralsPath = req.body.path.replace(
        /[^\/]+$/,
        (result.subGroup === "PatientRegistration"
          ? "config/Registration"
      ...
      if (fs.existsSync(referralsPath)) {
        result.referrals = JSON.parse(
          fs.readFileSync(referralsPath).toString("utf8")
        );
      }

      const orderPath = req.body.path.replace(/[^\/]+$/, "config/order.json");
  ...
  router.post("/programs/save_encounters", async function(req, res, next) {
    ...
    identifierType = idType.length > 0 ? idType[0].patientIdentifierTypeId : 3;
    ...
    /*(json.date
      ? new Date(json.date)
      : new Date()).format("d/mmm/YYYY")*/ const programName =
      json.program;

    const group = json.group;

    const currentUser = json.user;
    const currentLocationName = json.location ? json.location : "Unknown";
    ...
    let encounterName =
      Object.keys(json["HTS Visit"]).indexOf("Result Given to Client") >= 0 &&
      ["Confirmatory Positive", "Confirmatory (Antibody) Positive"].indexOf(
        json["HTS Visit"]["Result Given to Client"]
      ) >= 0
    ...
    encounterType = encType ? encType.encounterTypeId : null;
    ...
    clinicId = await generateEntryCode(
      patientId,
      currentUser,
      currentLocationName,
      "EC",
    ...
    const ageInDays =
      (new Date(today) - new Date(dateOfBirth)) / (24.0 * 60.0 * 60.0 * 1000.0);
    ...
    Object.keys(json[encounterName]).forEach(async name => {
      ...
      let value =
        json[encounterName][
          name === "Last HIV Test" ? "Last HIV Test Result" : name
        ];
      ...
      let valueCodedId = valueCoded ? valueCoded.conceptId : null;

      let valueCodedNameId = valueCoded ? valueCoded.conceptNameId : null;

      if (conceptId) {
        let obs;

        if (
          ["age", "time since last hiv test", "time since last test"].indexOf(
            name.toLowerCase()
          ) >= 0
        ) {
          ...
          ["appointment date given"].indexOf(name.toLowerCase()) >= 0
        ) {
          obs = await Obs.create({
            personId: patientId,
            conceptId,
          ...
          obs = await Obs.create({
            ...
            obsDatetime: new Date(today),
            locationId,
            valueCoded: valueCodedId ? valueCodedId : null,
            valueCodedNameId,
            valueNumeric:
              !valueCodedId &&
              String(value)
        ...
        let age =
          (new Date(today) - new Date(dateOfBirth)) /
          (365.0 * 24.0 * 60.0 * 60.0 * 1000.0);
  ...
  router.get("/programs/fetch/:id", async function(req, res, next) {
    ...
    if (!identifier) return res.status(200).json([]);

    let primaryId, patientId, clinicId, idType, identifierType, nhid;
    ...
    new client().get(
      ...
      function(result) {
        ...
        let json = result && result._source ? result._source : {};
  ...
  router.post("/programs/save_bd_data", async function(req, res, next) {
    ...
    let currentUser = json["Current User"] ? json["Current User"] : null;
    ...
    if (json["Entry Code"]) clinicId = json["Entry Code"];
    ...
    if (!json.birthdate && json["Age"]) {
      ...
      if (parts) {
        ...
        json.birthdate =
          new Date().getFullYear() - parseInt(json["Age"], 10) + "-07-10";
    ...
    if (!provider) {
      return res
        .status(400)
        .json({ error: true, message: "HTS Provider ID \n does not exist" });
    }
    ...
    let parts = json[name].trim().match(/^(([^-]+)-([^\(]+))\(([^\)]+)\)/);

    if (!parts)
      return res
        .status(400)
        .json({ error: true, message: "Error occured when picking locations" });
    ...
    let age =
      (new Date(today) - new Date(birthdate)) /
      (365.0 * 24.0 * 60.0 * 60.0 * 1000.0);
    ...
    let encounterName =
      Object.keys(json).indexOf("Result Given to Client") >= 0 &&
      ["Confirmatory Positive", "Confirmatory (Antibody) Positive"].indexOf(
        json["Result Given to Client"]
      ) >= 0
        ? "Confirmatory HIV Testing"
        : "HTS Visit";
    ...
    let encounterType = encType ? encType.encounterTypeId : null;
    ...
    let registerMap = await HtsRegisterEncounterMapping.create({
      encounterId,
      registerId
    });
    ...
    groups[encounterName].forEach(async name => {
      if (name === "HIV Rapid Test Outcomes") {
        Object.keys(json[name]).forEach(e => {
          Object.keys(json[name][e]).forEach(async f => {
        ...
        if (conceptId && value && String(value).length > 0) {
          ...
          if (
            ...
            ["appointment date given"].indexOf(conceptname.toLowerCase()) >= 0
          ) {
            obs = await Obs.create({
              personId: patientId,
              conceptId,
    ...
    if (
      ["New Negative", "New Positive"].indexOf(
        json["Result Given to Client"]
      ) >= 0
    ) {
      let htsSetting = "";
    ...
    new client().get(
      ...
      function(result) {
        ...
        if (
          result &&
          result.aggregations &&
          result.aggregations.kits &&
          result.aggregations.kits.buckets &&
          result.aggregations.kits.buckets.length > 0 &&
          result.hits &&
          result.hits.hits &&
          ...
          result.hits.hits
            ...
            .forEach(data => {
              ...
              if (row.visitDate) {
                json["Testing Date"] = new Date(row.visitDate).format(
                  "d mmm YYYY"
                );
              }
              ...
              if (row.observation.match(/first\spass/i)) {
                ...
                if (row.observation.match(/test\s1/i)) {
                  json["HIV Rapid Test Outcomes"]["First Pass"]["Test 1"] =
                    ["Non-Reactive", "Non-reactive"].indexOf(
                      row.observationValue
                    ) >= 0
                      ? "Non-Reactive"
                      : row.observationValue;
                } else if (row.observation.match(/test\s2/i)) {
                  json["HIV Rapid Test Outcomes"]["First Pass"]["Test 2"] =
                    ["Non-Reactive", "Non-reactive"].indexOf(
                      row.observationValue
                    ) >= 0
                      ? "Non-Reactive"
                ...
                row.observation === "Last HIV Test Result" ||
                row.observation === "Last HIV test"
              ) {
                ...
                json["Number of Items Given:Condoms:Female"] = String(
                  row.observationValue
                );
              } else if (row.observation === "Number of male condoms given") {
                json["Number of Items Given:Condoms:Male"] = String(
                  row.observationValue
                );
              } else if (
                row.observation === "Partner HIV Status" &&
                String(row.observationValue)
                  .toLowerCase()
                  .trim() === "no partner"
              ) {
                ...
                String(row.observation)
                  .toLowerCase()
                  .trim() === "referral for re-testing"
              ) {
                json["Referral for Re-Testing"] =
                  ["No Re-Test needed", "No Re-test needed"].indexOf(
                    row.observationValue
                  ) >= 0
                    ? "No Re-Test needed"
                    : ["Re-Test", "Re-test"].indexOf(row.observationValue)
                    ? "Re-Test"
                    : row.observationValue;
              } else {
  ...
  router.get("/programs/scan/:id", async function(req, res, next) {
    ...
    let result = !Array.isArray(ddeData) ? [ddeData] : ddeData;
  ...
  router.get("/programs/fetch_visits/:id", async function(req, res, next) {
    ...
    if (req.params.id === null) return res.status(200).json({});
  ...
  router.post("/programs/save_register_number", async function(req, res, next) {
    ...
    let entryCode = json.client ? Object.keys(json.client)[0] : null;
          ...
          if (result && result.hits && result.hits.hits) {
            result.hits.hits.forEach(async hit => {
              ...
              if (existingMap.length <= 0) {
                ...
                await HtsRegisterEncounterMapping.create({
                  encounterId: hit._source.encounterId,
                  registerId
                });
              }
              ...
              args = {
                data: Object.assign({}, hit._source, {
                  registerNumber,
                  locationType,
                  serviceDeliveryPoint
                }),
    ...
    let birthdate = person.birthdate ? person.birthdate : new Date();
    ...
    if (
      json.client[entryCode] &&
      ["New Negative", "New Positive"].indexOf(
        json.client[entryCode]["HTS Visit"]["Result Given to Client"]
      ) >= 0
    ) {
      let htsSetting = "";
      let htsModality = "";
      ...
      let partnerHIVStatus =
        json.client[entryCode]["HTS Visit"]["Partner HIV Status"];

      let referrer = json.client[entryCode]["HTS Visit"]["Who referred slip"];

      let gender = json.client[entryCode]["HTS Visit"]["Sex/Pregnancy"]
        ? String(json.client[entryCode]["HTS Visit"]["Sex/Pregnancy"])
            .substring(0, 1)
            .toUpperCase()
        : null;
      ...
      let clinicId = entryCode ? entryCode : null;
      ...
      new client().get(
        ...
        function(result) {
          ...
          let args = {
            data: {
              htsAccessType:
                htsAccessTypeMappings[
                  json.client[entryCode]["HTS Visit"]["HTS Access Type"]
                ],
              resultGiven: String(
                json.client[entryCode]["HTS Visit"]["Result Given to Client"]
              )
    ...
    res.status(200).json({ registerNumber, entryCode });
  });

  router.post("/programs/void_multiple_encounters", async function(
    req,
    res,
    next
  ) {
    ...
    encounterIds = await Encounter.find({
      where: {
    ...
    if (req.body.partnerId) {
      ...
      encounterIds = encounterIds.concat(
        await Encounter.find({
          where: {
            encounterDatetime: req.body.visitDate,
            patientId: patient.patientId
          }
  ...
  router.post("/programs/add_register", async function(req, res, next) {
    ...
    const json = req.body["Add Register"] ? req.body["Add Register"] : {};
    ...
    const serviceDeliveryPointId = serviceDeliveryPoint
      ? serviceDeliveryPoint.serviceDeliveryPointId
      : null;
    ...
    if (
      !existingRegister ||
      (existingRegister && Object.keys(existingRegister).length <= 0)
    ) {
      let register = await HtsRegister.create({
        registerNumber:
          json["Register Number"] + "-" + json["Service Delivery Point"],
        locationTypeId,
      ...
      return res
        ...
        .json({
          error: true,
          message: "Active register with same number already exists!"
        });
    }
  });

  router.post("/programs/close_register", async function(req, res, next) {
    const json = req.body["Close Register"] ? req.body["Close Register"] : {};
    ...
    if (existingRegister && Object.keys(existingRegister).length > 0) {
      ...
      new client().delete(
        es.protocol +
          ...
          es.index +
          "/register/" +
          String(registerNumber).replace(/\//g, "_"),
        function(result) {
          debug(result, null, 2);
  ...
  router.get("/programs/fetch_active_registers", function(req, res, next) {
    ...
    const args = {
      data: {
        query: {
          query_string: {
            query: (query.q ? "registerNumber:" + query.q : "") + "*"
          }
        }
      },
      headers: {
    ...
    new client().get(
      ...
      function(result) {
        ...
        if (
          result &&
          result.aggregations &&
          result.aggregations.registerNumber
        ) {
                    ...
                    result.hits.hits
                      ...
                      .forEach(data => {
                        ...
                        if (row.observation.match(/first\spass/i)) {
                          ...
                          row.observation.match(/immediate\srepeat/i)
                        ) {
                          if (!json["HIV Rapid Test Outcomes"]) {
                            json["HIV Rapid Test Outcomes"] = {};
                          }
                          ...
                          row.observation === "HTS Family Referral Slips"
                        ) {
                          json[
                            "Number of Items Given:HTS Family Referral Slips"
                          ] = String(row.observationValue);
                        } else if (
                          row.observation === "Number of female condoms given"
                        ) {
                          json["Number of Items Given:Condoms:Female"] = String(
                            row.observationValue
                          );
                        } else if (
                          row.observation === "Number of male condoms given"
                        ) {
                          json["Number of Items Given:Condoms:Male"] = String(
                            row.observationValue
                          );
                        } else if (
                          row.observation === "Time since last HIV Test"
                        ) {
  ...
  const loadJSON = result => {
    ...
    result.hits.hits.forEach(data => {
      ...
      if (row.observation.match(/first\spass/i)) {
        ...
        if (row.observation.match(/test\s1/i)) {
          json["HIV Rapid Test Outcomes"]["First Pass"]["Test 1"] =
            ["Non-Reactive", "Non-reactive"].indexOf(row.observationValue) >= 0
              ? "Non-Reactive"
              : row.observationValue;
        } else if (row.observation.match(/test\s2/i)) {
          json["HIV Rapid Test Outcomes"]["First Pass"]["Test 2"] =
        ...
        if (row.observation.match(/test\s1/i)) {
          json["HIV Rapid Test Outcomes"]["Immediate Repeat"]["Test 1"] =
            ["Non-Reactive", "Non-reactive"].indexOf(row.observationValue) >= 0
              ? "Non-Reactive"
              : row.observationValue;
        } else if (row.observation.match(/test\s2/i)) {
          json["HIV Rapid Test Outcomes"]["Immediate Repeat"]["Test 2"] =
            ["Non-Reactive", "Non-reactive"].indexOf(row.observationValue) >= 0
              ? "Non-Reactive"
              : row.observationValue;
        }
        ...
        json["Number of Items Given:HTS Family Referral Slips"] = String(
          row.observationValue
        );
      } else if (row.observation === "Number of female condoms given") {
        json["Number of Items Given:Condoms:Female"] = String(
      ...
      function(result) {
        ...
        new client().get(
          ...
          function(result) {
            ...
            const indices = result.aggregations.registerNumber.buckets[0].entryCode.buckets.map(
              e => {
                return e.key;
              }
            );
  ...
  router.post("/programs/save_edit_data", async (request, response) => {
    ...
    const USER = await Users.findOne({ where: { username: request.body.activeUser } })

    if (Object.keys(PAYLOAD).length && USER) {
      try {
        const entryCode = await Obs.findOne({ where: { valueText: ENTRY_CODE } })

        // find and void existing observations in MySQL then create new observations
        for (let concept of Object.keys(PAYLOAD)) {
          const openMrsConcept = await ConceptName.findOne({ where: { name: concept } })
          const observation = await Obs.findOne({ where: { personId: entryCode.personId, conceptId: openMrsConcept.conceptId, encounterId: entryCode.encounterId, voided: 0 } })

          if (observation) {
            let newObservation = observation
            await Obs.upsertWithWhere({ obsId: observation.obsId }, { voided: 1, voidedBy: USER.userId, voidReason: 'Voided by user through update' })

            newObservation.valueCoded = (await ConceptName.findOne({ where: { name: PAYLOAD[concept] } })).conceptId
            ...
            await Obs.create(newObservation)
          }
        }
  ...
  router.get("/set_location/:location/:token", function(req, res, next) {
    ...
    validActiveToken(req.params.token)
      .then(userId => {
        debug(userId);
                ...
                Location.findOne(
                  ...
                  (err, result) => {
                    if (err || !result) {
                      ...
                      res.status(200).json({ location: result.name });
                    }
                  }
                );
              } else {
  ...
  router.get("/programs/list_service_delivery_points", function(
    req,
    res,
    next
  ) {
  ...
  router.post("/programs/update_partner_record", async function(
    req,
    res,
    next
  ) {
    ...
    const obs = await Obs.findOne({
      where: {
        ...
        obsDatetime: {
          gt: new Date(
            new Date(req.body.visitDate).setDate(
              new Date(req.body.visitDate).getDate() - 1
            )
          ),
          lt: new Date(
            new Date(req.body.visitDate).setDate(
    ...
    if (obs) {
      ...
      const entryCodeConceptId = entryCodeConcept
        ? entryCodeConcept.conceptId
        : null;

      const clinicIdentifier = entryCodeConceptId
        ? await Obs.findOne({
            where: {
              ...
              obsDatetime: {
                gt: new Date(
                  new Date(obs.obsDatetime).setDate(
                    new Date(obs.obsDatetime).getDate() - 1
                  )
                ),
                lt: new Date(
                  new Date(obs.obsDatetime).setDate(
      ...
      const registerConceptId = registerConcept
        ? registerConcept.conceptId
        : null;

      const registerObs = registerConceptId
        ? await Obs.findOne({
            where: {
        ...
        async function(result) {
          ...
          if (registerNumber !== null) {
            ...
            if (register) {
              ...
              if (locType) locationType = locType.name;
          ...
          const age = person
            ? new Date(person.birthdate) / (365.0 * 24.0 * 60.0 * 60.0 * 1000.0)
            : null;
  ...
  router.get("/programs/fetch_locations", function(req, res, next) {
    ...
    new client().get(
      ...
      function(result) {
        ...
        if (
          Object.keys(result).indexOf("aggregations") >= 0 &&
          Object.keys(result.aggregations).indexOf("locations") >= 0 &&
          Object.keys(result.aggregations.locations).indexOf("buckets") >= 0 &&
          result.aggregations.locations.buckets.length > 0
        ) {
  ...
  router.post("/add_location", async function(req, res, next) {
    ...
    const name =
      req.body &&
      req.body["Add Location"] &&
      req.body["Add Location"]["Location Name"]
        ? req.body["Add Location"]["Location Name"]
        : undefined;
    ...
    if (name === undefined)
      return res
        .status(400)
        .json({ error: true, message: "No valid location provided!" });
    ...
    if (existing)
      return res
        .status(400)
        .json({ error: true, message: "Location already exists!" });
  ...
  router.post("/add_village", async function(req, res, next) {
    ...
    if (!existingDistrict) {
      ...
      if (!result)
        return res
          .status(400)
          .json({ error: true, message: "Failed to save District!" });
    ...
    if (!existingTA) {
      ...
      if (!result)
        return res
          .status(400)
          .json({ error: true, message: "Failed to save T/A!" });

      traditionalAuthorityId = result ? result.traditionalAuthorityId : null;

      isDirty = true;
    } else {
      traditionalAuthorityId = existingTA
        ? existingTA.traditionalAuthorityId
        : null;
    }
    ...
    if (!existingVillage) {
      ...
      if (!result)
        return res
          .status(400)
          .json({ error: true, message: "Failed to save village!" });
  ...
  router.get("/redirect_to_portal", async function(req, res, next) {
    ...
    const site = fs.existsSync(filename)
    ...
    res
      .status(200)
      .json({
        redirect_to_portal: site.redirect_to_portal,
  ...
  router.post("/programs/save_referral_outcome", async function(
    req,
    res,
    next
  ) {
    ...
    const userId =
      user && Array.isArray(user) && user.length > 0 ? user[0].userId : null;
    ...
    [
      ...
      if (req.body[name] && encounterId !== null && userId !== null) {
        ...
        const conceptId =
          concept && Array.isArray(concept) && concept.length > 0
            ? concept[0].conceptId
            : null;
        ...
        if (conceptId !== null) {
          ...
          switch (name) {
            ...
            case "Referral Outcome":
              const conceptName = await ConceptName.find({
                where: { name: req.body[name] }
              });

              if (
                conceptName &&
                Array.isArray(conceptName) &&

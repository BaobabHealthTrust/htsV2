...
class FindEnteredRecord extends Component {
  ...
  handleCellClick(fieldType, category, field, subField, subSubField, j, i, group, e) {
    ...
    if (category && field && subField && subSubField && fieldType.type === "click") {
      ...
      if (!newState.data[category][field][subField])
  ...
  validated(data) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!String(data.Age).trim().match(/\d+[D|W|M|Y]$/i)) {
      ...
      if (age < constraints.ageGroup[data["Age Group"]].min || age > constraints.ageGroup[data["Age Group"]].max) {
      ...
      if (String(data["Last HIV Test"]).trim() !== "Never Tested" && !data["Time Since Last Test"]) {
      ...
      } else if (String(data["Last HIV Test"]).trim() !== "Never Tested" && !String(data["Time Since Last Test"]).match(/\d+[D|W|M|Y]$/)) {
  ...
  async handleSave() {
    ...
    this
      ...
      .then(() => {
        ...
        this
          .props
          .saveEditRow("/programs/save_edit_data", {
            full: workingData,
            changes: this.state.data,
            activeUser: (this.props.app && this.props.app.activeUser
          ...
          .then(async () => {
            ...
            this
              .props
              .showInfoMsg("Done", "Changes saved!");
          ...
          .catch(() => {
            ...
            this
              .props
              .showErrorMsg("Error", "Saving changes failed");
  ...
  componentDidMount() {
    ...
    let id = (this.props.wf && this.props.activeWorkflow && this.props.wf.responses && this.props.wf.responses[this.props.activeWorkflow]
      ? (this.props.wf.responses[this.props.activeWorkflow]['Entry Code'])
      : null);

    this
      .props
      .fetchEditRow("/programs/fetch_edit_record/" + id);

    this
      .props
      .updateApp({ sectionHeader: "Edit Entered Record" });
  ...
  render() {
    ...
    return (
      ...
      <div className="mainContainerFind">
        <table width="100%">
          <tbody>
            <tr>
              <td style={{
                                        ...
                                        ? (j === 1
                                          ? <div
                                            ...
                                            className={(fieldNames[i] && fieldNames[i].category && fieldNames[i].field && fieldNames[i].subField && fieldNames[i].subSubField && this.state.data[fieldNames[i].category] && this.state.data[fieldNames[i].category][fieldNames[i].field] && this.state.data[fieldNames[i].category][fieldNames[i].field][fieldNames[i].subField] === fieldNames[i].subSubField)
                                              ? "circled"
                                              : ((this.props.current && fieldNames[i] && fieldNames[i].category && fieldNames[i].field && fieldNames[i].subField && ((fieldNames[i].subSubField && this.props.current[fieldNames[i].category] && this.props.current[fieldNames[i].category][fieldNames[i].field] && this.props.current[fieldNames[i].category][fieldNames[i].field][fieldNames[i].subField] === fieldNames[i].subSubField) || (this.props.current[fieldNames[i].category] === fieldNames[i].field)))
                                                ? "circledFind"
                                                : (fieldNames[i] && fieldNames[i].category && fieldNames[i].field && this.state.data[fieldNames[i].category] === fieldNames[i].field
                                                  ? "circled"
                                                  : (fieldNames[i] && fieldNames[i].category && fieldNames[i].field && this.props.current[fieldNames[i].category] === fieldNames[i].field && !this.state.data[fieldNames[i].category]
                                                    ? "circledFind"
                                                    : "normal")))}>{fields[i]
                                                      ? fields[i]
                                                      : ""}</div>
...
export default FindEnteredRecord;
